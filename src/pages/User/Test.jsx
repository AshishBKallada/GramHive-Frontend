...
function WebcamSample({ handleBack }) {
        ...
        try {
            ...
            if (response.status === 200) {
                setTimeout(() => {
                    ...
                    toast.success('story uploaded', { autoClose: 1000, onClose: () => { setLoading(false); navigate('/profile') } })
                }, 5000)
            }
    ...
    if (vedioRecorder) {
        return (
            (<><S3VideoRecorder handleUploadStory={handleUploadStory} handleBack={setVedioRecorder} /></>)
        )
    }
    if (capturedPhoto) {
        return (
