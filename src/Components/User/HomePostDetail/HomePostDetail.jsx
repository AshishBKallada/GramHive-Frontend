...
function HomePostDetail({ post, setCurrPost, setPosts, posts }) {
    ...
    const handleComment = async (e) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3000/comment/${postId}/comments`);
        ...
        const fetchLikes = async () => {
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (response.data.likes.length > 0 && response.data.likes.some((like) => like.user._id === author)) {
    ...
    const handleLike = async () => {
        try {
            if (!liked) {
                ...
                await axios.post(`http://localhost:3000/posts/${postId}/like`, { author });
                setLikes(prevLikes => prevLikes + 1);
            } else {
                await axios.delete(`http://localhost:3000/posts/${postId}/unlike`, { author });
                setLikes(prevLikes => prevLikes - 1);
            }
            setLiked(!liked);
        } catch (error) {
    ...
    const handleSave = async (e) => {
        try {
            ...
            const response = await fetch(`http://localhost:3000/posts/${postId}/save/${author}`, { method: 'POST' });
            if (response.ok) {
                ...
                console.log(post, '_______________');
                setIsSaved(true);
            }
        } catch (error) {
            console.error(error)
    ...
    const handleUnSave = async (e) => {
        try {
            console.log(postId, author)
            const response = await fetch(`http://localhost:3000/posts/${postId}/unsave/${author}`, { method: 'POST' });
            if (response.ok) {
                ...
                setPosts(updatedPosts)
                setIsSaved(!isSaved);
                toast.success('Post UnSaved', { autoClose: 1000 })
            }
        } catch (error) {
            console.error(error)
    ...
    const handleReport = async () => {
        try {
            ...
            const response = await axios.post(`http://localhost:3000/posts/report/${postId}`, {
                author: author,
                userId: userId
            });
                        ...
                        <div className="grid grid-cols-12 gap-4 h-full">
                            ...
                            <div className="col-span-12 md:col-span-6 xl:col-span-6">
                                ...
                                <div class="flex items-center px-4 py-3">
                                    ...
                                    <img class="h-8 w-8 rounded-full" src={post.userId.image} />
                                    <div class="ml-3 flex-grow">
                                        <span class="text-sm font-semibold antialiased block leading-tight">{post.userId.username}</span>
                                            ...
                                            <div class="bg-white rounded-lg md:max-w-md md:mx-auto p-4 fixed inset-x-0 bottom-0 z-50 mb-4 mx-4 md:relative shadow-lg">
                                                <div class="md:flex items-center">
                                                    ...
                                                    <div class="mt-4 md:mt-0 md:ml-6 text-center md:text-left">
                                                        ...
                                                        <p class="text-sm text-gray-700 mt-1">Are you sure you want to report this post? Reporting helps us maintain a safe and respectful community. If you believe this post violates our community guidelines, please proceed with reporting.

                                                        </p>
                                                    </div>
                                                </div>
                                                <div class="text-center md:text-right mt-4 md:flex md:justify-end">
                                                    <button onClick={handleReport} id="confirm-delete-btn" class="block w-full md:inline-block md:w-auto px-4 py-3 md:py-2 bg-red-200 text-red-700 rounded-lg font-semibold text-sm md:ml-2 md:order-2">
                                                        Report
                                                    </button>
                                                    <button onClick={() => setReportModal(false)} id="confirm-cancel-btn" class="block w-full md:inline-block md:w-auto px-4 py-3 md:py-2 bg-gray-200 rounded-lg font-semibold text-sm mt-4 md:mt-0 md:order-1">
                                                        Cancel
                                                    </button>
                                                </div>
                                            </div>
                                ...
                                {showLike && <LikedList likes={likes} closemodal={setShowLike} />}
                                ...
                                <div class="flex items-center justify-between mx-4 mt-3 mb-2">
                                    <div class="flex gap-5">
                                        <button onClick={handleLike}>
                                            {liked ? (
                                                <svg fill="#ff0000" height="24" viewBox="0 0 48 48" width="24">
                                    ...
                                    <div class="flex">
                                        {isSaved ?
                                            ...
                                            : <button onClick={handleSave}> <svg fill="#262626" height="24" viewBox="0 0 48 48" width="24"><path d="M43.5 48c-.4 0-.8-.2-1.1-.4L24 29 5.6 47.6c-.4.4-1.1.6-1.6.3-.6-.2-1-.8-1-1.4v-45C3 .7 3.7 0 4.5 0h39c.8 0 1.5.7 1.5 1.5v45c0 .6-.4 1.2-.9 1.4-.2.1-.4.1-.6.1zM24 26c.8 0 1.6.3 2.2.9l15.8 16V3H6v39.9l15.8-16c.6-.6 1.4-.9 2.2-.9z"></path></svg>
                                            </button>
                                        }
                                    </div>
                                </div>
                                <div onClick={() => setShowLike(prev => !prev)} class="font-semibold text-sm mx-4 mt-2 mb-4">{likes.length} likes
                                </div>

                            </div>

                            {/* Right Column */}
                            <div class="col-span-12 md:col-span-6 xl:col-span-6 " style={{ maxHeight: '500px' }}>
                                <h2 class="text-lg font-bold mb-2">Comments</h2>
                                {comments.length > 0 ? (
                                    <div class="bg-white shadow px-3 py-2 flex flex-col space-y-2 overflow-y-auto" style={{ maxHeight: '380px', minHeight: '380px' }}>

                                        {comments.map((comment, index) => (

                                            <div key={comment._id} class="flex items-center space-x-2">
                                                <div class="group relative flex flex-shrink-0 self-start cursor-pointer">
                                                    <img
                                                        src={comment.author?.image} alt="" class="h-8 w-8 object-fill rounded-full" />
                                                ...
                                                <div class="flex items-center justify-center space-x-2">
                                                    <div class="block">
                                                        ...
                                                        {comment.replies.length > 0 && comment.replies.map((r, index) => (
                                                            <div key={index} class="flex items-center space-x-2 space-y-2">
                                                                <div class="group relative flex flex-shrink-0 self-start cursor-pointer pt-2">
                                                                    <img src={reply.author?.image} alt="" class="h-8 w-8 object-fill rounded-full" />
                                                                </div>
                                                                <div class="flex items-center justify-center space-x-2">
                                                                    <div class="block">
                                                                        <div class="bg-gray-100 w-auto rounded-xl px-2 pb-2">
                                                                        ...
                                                                        <div class="flex justify-start items-center text-xs w-full">
                                                                            <div class="font-semibold text-gray-700 px-2 flex items-center justify-center space-x-1">
                                                                                ...
                                                                                <a href="#" className="hover:underline">
                                                                                    <small>{formatDistanceToNow(new Date(r.createdAt), { addSuffix: true })}</small>
                                                                                </a>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                            ...
                                                            <div class="flex items-center space-x-2 space-y-2">
                                                                <div class="group relative flex flex-shrink-0 self-start cursor-pointer pt-2">
                                                                    <img
                                                                        ...
                                                                        src={user.image} alt="" class="h-8 w-8 object-fill rounded-full" />
                                ...
                                <div class="mt-6 p-4 bg-gray-100 flex items-center">
                                    {replyingTo ? (
                                        <textarea ref={replyTextareaRef}
                                            value={reply} onChange={(e) => setReply(e.target.value)} class="flex-grow h-12 border-gray-300 rounded-md p-2" placeholder="Write your reply here..."></textarea>
                                    ) : (
                                        <textarea value={comment} onChange={(e) => setComment(e.target.value)} class="flex-grow h-12 border-gray-300 rounded-md p-2" placeholder="Write your comment here..."></textarea>
                                    )}
                                    <button onClick={replyingTo ? handleReply : handleComment} class="px-4 py-2 ml-4 bg-blue-500 text-white rounded-md hover:bg-blue-800">
