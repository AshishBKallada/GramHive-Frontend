...
function HomePostDetail({ post, setCurrPost, setPosts, posts }) {
  ...
  const handleComment = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/comment/${postId}/addcomments`,
        {
          comment,
          author,
        }
  ...
  const handleSave = async () => {
    const response = await savePost(postId, author);
    if (response.data.success) {
      setIsSaved(true);
      const updatedPosts = posts.map((post) => {
        return post._id === postId ? { ...post, isSaved: true } : post;
      });
  ...
  const handleUnSave = async () => {
    const response = await unsavePost(postId, author);
    if (response.data.success) {
      setIsSaved(false);
      const updatedPosts = posts.map((post) => {
        return post._id === postId ? { ...post, isSaved: false } : post;
      });
      setPosts(updatedPosts);
    }
  };
              ...
              <div className="col-span-12 md:col-span-6 xl:col-span-6">
                ...
                <div class="flex items-center justify-between mx-4 mt-3 mb-2">
                  <div class="flex gap-5">
                    {post &&
                    post?.likes?.find(
                      (like) => like.user._id.toString() === author.toString()
                    ) ? (
                ...
                {comments.length > 0 ? (
                  ...
                  >
                    {comments.map((comment, index) => (
                      <div key={comment._id} class="flex items-center">
