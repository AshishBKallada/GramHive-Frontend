  ...
  const handleUpload = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/posts/addpost/${userId}`,
        {
          method: "POST",
          body: formData,
          credentials: "include",
        ...
        <DialogBody>
          ...
          <div className="flex relative ml-36">
            {images.map((image, index) => (
              <div
                key={index}
                className={`w-44 h-12 absolute rounded-lg border border-blue-800 shadow-md transform transition-all skew-x-8 -skew-y-12 -top-${
                  index * 4
                } -left-${index * 4}`}
              >
                <img
