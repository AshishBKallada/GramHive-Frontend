...
const GoogleAuth = ({ type, setUserData }) => {
  ...
  useEffect(() => {
    loadAuth2(window.gapi, '502311807627-emfijnj80jj7t6qvqjlfu0jagndimeej.apps.googleusercontent.com', 'profile email')
      .then((auth2) => {
  ...
  const responseGoogle = (response) => {
    ...
    const { wt: { Ad: username, cu: email }, tokenObj: { access_token: password }, profileObj: { imageUrl: image } } = response;
    if (type === 'signup') {
      setUserData({
        next: true,
        name: username,
  ...
  return (
    <div className="flex items-center justify-center mt-4">
      <GoogleLogin
        ...
        render={renderProps => (
          <button onClick={renderProps.onClick} disabled={renderProps.disabled} className="hover:scale-105 ease-in-out duration-300 shadow-lg p-2 rounded-lg m-1">
