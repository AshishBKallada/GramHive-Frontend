...
function PostCard({ posts, setPosts,likeMap,setLikeMap }) {
    const author = useSelector((state) => state.user.user._id);
    const [currPost, setCurrPost] = useState(null);
    console.log(likeMap,'________________')
    ...
    return (
        <div>
            <ToastContainer />
            {currPost && <HomePostDetail post={currPost} setCurrPost={setCurrPost} setPosts={setPosts} posts={posts} />}
            {posts.map((post, i) =>
                <div key={post._id} className='mt-5 p-4'>
                    <div className="bg-white border rounded-sm max-w-xl">
                        <div className="flex items-center px-4 py-3">
                            <img className="h-8 w-8 rounded-full" src={post.userId.image} alt={post.userId.username} />
                            <div className="ml-3">
                                <span className="text-sm font-semibold antialiased block leading-tight">{post.userId.username}</span>
                        ...
                        <div className="h-56 sm:h-64 xl:h-80 2xl:h-96">
                            <Carousel slideInterval={1500}>
                                {post.images.map((image, index) => (
                                    <img onClick={() => handlePostDetails(post)} key={index} src={image} alt={`Image ${index}`} />
                                ))}
                            </Carousel>
                        </div>
                        ...
                        <div className="flex items-center justify-between mx-4 mt-3 mb-2">
                            <div className="flex gap-5">
                                ...
                                <svg onClick={() => handlePostDetails(post)} fill="#262626" height="24" viewBox="0 0 48 48" width="24">
